...
export class Creature extends Entity {
    ...
    private stdAnimate(rightHandTargets: PIXI.Point[], leftHandTargets: PIXI.Point[], dt: number) {
        if(near(this.leftHandTarget, this.leftHand.position as PIXI.Point, this.animateNear) && near(this.rightHand.position as PIXI.Point, this.rightHandTarget, this.animateNear)) {
            ++this.animateState;
    ...
    private moveHand(hand: PIXI.Container, target: PIXI.Point, dt: number) {
        let diff = sub(target, hand.position as PIXI.Point);
        let mov = scale(diff, 0.5 * dt * 10);
        hand.position = add(hand.position as PIXI.Point, mov);
    }
}
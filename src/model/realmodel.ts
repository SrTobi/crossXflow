...
function setupTileNodes(tmpl: Model.ITile, pos: Vector): Tile {
  if (tmpl.type === Model.TileType.Street) {
    ...
    const vnw = connect(topMin, vMidMin, EdgeShapes.Straight, 2);
    const vsw = connect(vMidMin, bottomMin);
    const vse = connect(bottomMax, vMidMax, EdgeShapes.Straight, 2);
    const vne = connect(vMidMax, topMax);
    ...
    const hne = connect(rightMax, hMidMax, EdgeShapes.Straight, 2);
    const hnw = connect(hMidMax, leftMax);
    const hsw = connect(leftMin, hMidMin, EdgeShapes.Straight, 2);
    const hse = connect(hMidMin, rightMin);
    ...
    return new Tile(
      ...
      [vMidMax, vMidMin, hMidMin, hMidMax]
    );
  } else {
